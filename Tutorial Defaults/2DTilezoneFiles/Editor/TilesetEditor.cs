...
public class TilesetEditor : EditorWindow {
	...
	static void SetWireframeHidden ( bool isHidden ) {
		...
		UnityEditor.EditorUtility.SetSelectedRenderState( layerTileInfo.meshRenderer, UnityEditor.EditorSelectedRenderState.Hidden );
#else
...
//				bool isSelected = (autoTileSelected == i);
	...
	void DrawSideControls() {
		...
		if (toolSelected == TileTool.Collisions) {
			...
			if( layerTileInfo != null )
				layerTileInfo.borderPixelOffset = EditorGUILayout.Slider( "Border Pixel Offset", layerTileInfo.borderPixelOffset, 0, 1 );
